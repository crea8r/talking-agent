...
const PORT = Number.parseInt(process.env.PORT || '4385', 10);
...
server.on('error', (error) => {
  ...
  console.error('call-landing failed to start', error);
  process.exit(1);
});

server.listen(PORT, HOST, () => {
  console.log(`call-landing listening at http://${HOST}:${PORT}`);
});
